...
def invert(d):
    return dict(map(lambda p: (p[1], p[0]), d.items()))
...
class Code128(Barcode1D):
    ...
    def extra(self, value):
        ...
        types, data = (self.extra[0], self.extra[1:])
        self.__extra = self.class_for(types)(data)
    ...
    @property
    def checksum(self):
        return (self.start_num + sum(map(lambda pair: pair[1] * (pair[0] + 1),
                                         enumerate(self.numbers + self.extra_numbers)))) % 103

    @property
    def numbers(self):
        return list(map(lambda char: self.values[char], self.characters))
